...
void PairDeepMD::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < nall; ++ii){
    for (int dd = 0; dd < 3; ++dd){
      f[ii][dd] += fscale[1][1] * scale[1][1] * dforce[3*ii+dd];
    }
  }
  ...
  if (vflag) {
    virial[0] += 1.0 * dvirial[0] * scale[1][1] * fscale[1][1];
    virial[1] += 1.0 * dvirial[4] * scale[1][1] * fscale[1][1];
    virial[2] += 1.0 * dvirial[8] * scale[1][1] * fscale[1][1];
    virial[3] += 1.0 * dvirial[3] * scale[1][1] * fscale[1][1];
    virial[4] += 1.0 * dvirial[6] * scale[1][1] * fscale[1][1];
    virial[5] += 1.0 * dvirial[7] * scale[1][1] * fscale[1][1];
  }
}
...
double PairDeepMD::init_one(int i, int j)
{
  ...
  if (setflag[i][j] == 0) fscale[i][j] = 1.0;
