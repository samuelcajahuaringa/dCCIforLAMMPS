  ...
  for (jn = 0; jn < numneigh; jn++) {
    ...
    if (!iszero(scrfcn[fnoffset + jn]) && eltj >= 0) {
      ...
      if (rij2 < this->cutforcesq) {
        ...
        if (vflag_either) {
          fi[0] = (delij[0] * force + dUdrijm[0]) * fscaleij;
          fi[1] = (delij[1] * force + dUdrijm[1]) * fscaleij;
          fi[2] = (delij[2] * force + dUdrijm[2]) * fscaleij;
        ...
        for (kn = 0; kn < numneigh_full; kn++) {
          ...
          if (k != j && eltk >= 0) {
            ...
            if (!iszero(dsij1) || !iszero(dsij2)) {
              force1 = dUdsij * dsij1 * fscaleij;
              force2 = dUdsij * dsij2 * fscaleij;
