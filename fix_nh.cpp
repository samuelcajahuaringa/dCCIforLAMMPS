  ...
  etap_mass(NULL)
{
  ...
  while (iarg < narg) {
    ...
    } else if (strcmp(arg[iarg],"ners") == 0) {
      lambda_flag = 1;
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) {
        int n = strlen(&arg[iarg+1][2]) + 1;
        lstr = new char[n];
        strcpy(lstr,&arg[iarg+1][2]);
        lstyle = EQUAL;
      } else {
        lstyle = CONSTANT;
      }
...
void FixNH::init()
{
  ...
  if (lstr) {
    lvar = input->variable->find(lstr);
    if (lvar < 0)
      error->all(FLERR,"Scaling variable name ot exist");
    if (input->variable->equalstyle(lvar)) lstyle = EQUAL;
    else error->all(FLERR,"Scaling variable is invalid style");
  }
}
...
void FixNH::setup(int vflag)
{
  ...
  if (tstat_flag) {
    if (logistic_flag) {
      ...
      //eta_mass[0] = 1.0 / t_freq;//tdof * boltz * t_target / t_freq;
    } else {
...
double FixNH::compute_scalar()
{
  ...
  if (tstat_flag) {
    if (logistic_flag) {
      ...
      // p_eta = Q*eta_dot
      // Q = L*K*T/t_freq
      energy += ke_target * eta[0] - kt * logistic_term(eta_dot[0]) + kt * log(eta_mass[0]);
    } else {
...
//  if (lstyle == CONSTANT) {
    ...
    lambda = input->variable->compute_equal(lvar);
    if (lambda < 0.0)
      error->one(FLERR,
                 "Scaling variable must be > 0.0");
    modify->addstep_compute(update->ntimestep + nevery);
  }

 // if (comm->me == 0) printf("pressure %g\n",p_hydro);

  for (int i = 0; i < 3; i++)
    if (p_flag[i]) {
      f_omega = (p_current[i]-p_hydro*lambda)*volume /
